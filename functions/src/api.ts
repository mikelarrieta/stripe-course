...
// make our API accesible to other URLs.
app.use( cors({ origin: true }) );

// middleware to convert the incoming body from string to JSON object.
app.use( 
  express.json({
...
/**
 * Catch async erros when awaiting promises
 */
...
app.post('/test', (req: Request, res: Response) => {
  ...
  res.status(200).send({ with_tax: amount * 14 });
});
